...
This should be a **split-stack Linux desktop app**:
...
That is the most practical 2026 stack because it uses the strongest tool for each job instead of forcing one language across incompatible ecosystems.
...
1. **`fancontrold` (Rust, root, system service)**
   - Reads hwmon
   - Owns config and runtime state
   - Runs PID loops
   - Publishes DBus API
   - Owns all fail-safe behavior

2. **`kde-fan-control` GUI (Qt/QML + small C++ glue layer, unprivileged)**
   - Talks to daemon over DBus only
   - Never touches sysfs directly
   - Uses Kirigami for primary UX
   - Uses Qt DBus for method calls, signals, and properties

3. **`kfc` CLI (Rust, unprivileged by default)**
   - Talks to daemon over DBus only
   - Mirrors GUI capabilities for automation and debugging

This is better than a single-process app because it keeps **privilege, safety, and hardware writes** in one supervised process.

## systemd recommendations

Use a **system service** with:
...
Why `Type=notify` instead of `simple`:
...
`Type=dbus` is viable, but `Type=notify` is the better practical default because this daemon also needs non-DBus startup checks and watchdog signaling.
...
Use **raw sysfs + your own domain model** as the authoritative runtime path. Do **not** make `libsensors` your source of truth for control, because your app needs:
...
But do expect to borrow good ideas from the `lm-sensors` ecosystem for discovery and diagnostics.
...
```toml
[dependencies]
tokio = { version = "1.51.1", features = ["macros", "rt-multi-thread", "signal", "time", "fs"] }
...
Use distro Qt/KDE dev packages for:
...
If the goal is a **practical, shippable, KDE-native Linux fan-control app**, the best stack is:
...
Do **not** try to make the whole product “all Rust” just for stack purity. The backend should be Rust-first; the KDE GUI should be Qt/KDE-native.
